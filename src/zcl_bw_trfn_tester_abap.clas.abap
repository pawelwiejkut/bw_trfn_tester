CLASS zcl_bw_trfn_tester_abap DEFINITION
  PUBLIC
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    "! <p class="shorttext synchronized" lang="en"></p>
    "! Check is TRFN AMDP
    "! @parameter rv_amdp | <p class="shorttext synchronized" lang="en"></p>
    METHODS check_is_amdp
      RETURNING VALUE(rv_amdp) TYPE abap_bool.
...
CLASS zcl_bw_trfn_tester_abap IMPLEMENTATION.
...
*       et_table_mod  =
  ...
  METHOD check_is_amdp.

    DATA(lobj_msg) = NEW cl_rso_msg(  ).

    DATA(lt_tlogo) = VALUE rstran_t_tlogo( ( tlogo = 'TRFN' objnm = gv_trfnid ) ).
